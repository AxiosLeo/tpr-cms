...
class CodeLogic
{
    ...
    private static function ticket($category_uniq, $baseCount)
    {
        ...
        RedisClient::init()->redis()->watch($key);

        /*** @var \Redis $multi_incr ** */
        $multi_incr = RedisClient::init()->redis()->multi()->incr($key);
        $ret        = $multi_incr->exec();

        if ($ret === false) {
            RedisClient::init()->redis()->unwatch();
            RedisClient::init()->redis()->discard();
    ...
    private static function encoding($category_uniq, $ticket)
    {
        ...
        $result = RedisClient::init()->redis()->hSetNx($key, $code, $ticket);
        if ($result === false) {
