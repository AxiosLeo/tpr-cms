...
class CodeLogic
{
    ...
    private static function ticket($category_uniq, $baseCount)
    {
        ...
        DbRedis::init()->redis()->watch($key);

        /*** @var \Redis $multi_incr ** */
        $multi_incr = DbRedis::init()->redis()->multi()->incr($key);
        $ret        = $multi_incr->exec();

        if ($ret === false) {
            DbRedis::init()->redis()->unwatch();
            DbRedis::init()->redis()->discard();
    ...
    private static function encoding($category_uniq, $ticket)
    {
        ...
        $result = DbRedis::init()->redis()->hSetNx($key, $code, $ticket);
        if ($result === false) {
